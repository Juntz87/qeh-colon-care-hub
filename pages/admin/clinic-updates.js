...
export default function AdminClinicUpdates(){
  ...
  async function createUpdate(e){
    ...
    await addDoc(collection(db, 'clinic_updates'), { title, category, body, date: serverTimestamp(), createdBy: user?.email || null })
          ...
          <>
            ...
            <div className="mt-6">
              ...
              <div className="mt-2 space-y-2">
                {items.map(i => (
                  <div key={i.id} className="p-3 border rounded bg-gray-50 dark:bg-gray-700 flex justify-between">
                    <div>
                      ...
                      <div className="text-sm text-gray-600 dark:text-gray-300">{i.category}</div>
                    </div>
                ...
                {items.length===0 && <div className="text-gray-600 dark:text-gray-300">No updates yet.</div>}
              </div>
            </div>
          </>
        ) : (
          <div className="mt-4">
            <p className="text-gray-700 dark:text-gray-200">Please sign in with Google to edit clinic updates.</p>
            <button onClick={login} className="mt-3 px-4 py-2 bg-qehBlue text-white rounded">Sign in with Google</button>
          </div>
        )}
      </div>
    </Layout>
